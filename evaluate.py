...
class Evaluator():
    ...
    @classmethod
    def MI(cls, image_F, image_A, image_B, bins=256):
        cls.input_check(image_F, image_A, image_B)

        def mutual_information(x, y, bins):
            joint_hist, _, _ = np.histogram2d(x.ravel(), y.ravel(), bins=bins)
            joint_prob = joint_hist / np.sum(joint_hist)
            px = np.sum(joint_prob, axis=1)
            py = np.sum(joint_prob, axis=0)

            nzs = joint_prob > 0
            return np.sum(joint_prob[nzs] * np.log(joint_prob[nzs] / (px[:, None] * py[None, :])[nzs]))

        mi_FA = mutual_information(image_F, image_A, bins)
        mi_FB = mutual_information(image_F, image_B, bins)
        return mi_FA + mi_FB
